...
int main(int argc, char* argv[]) {
	...
	/* !!!! VM1 main code !!!! */
	sendFrameTo(target, 0xFFFD, "Hello, I'm 2015130741 WonHyejin", strlen("Hello, I'm 2015130741 WonHyejin"));
...
void receiveARPFrame(unsigned char* dst, unsigned char* arp) {
	...
	receivedId = (unsigned char*)malloc(strlen(ra->data)+1); // identifier 길이 + \0 길이
	strcpy(receivedId, ra->data); // strcpy를 실행할 경우, str의 맨 마지막에 \0이 함께 복사되므로 memcpy대신 strcpy 사용

	if (strcmp(name, receivedId)) {
		printf("Received Target name is not mine. ignore..\n");
		return;
	}
	printf("Received Target name is mine. send ARP Reply..\n");

	int name_len = strlen(name);
	mah = (struct myarp_header*)malloc(sizeof(struct myarp_header) + name_len);
	...
	mah->len = name_len;
	memcpy(mah->ethAddr, STATION_ADDR, sizeof(STATION_ADDR)); //STATIN_ADDR : socket_init 과정에서 실행되는 기기의 MAC주소가 할당된 상태
	memcpy(mah->data, name, name_len);

	sendFrame(dst, 0xFFFE, (unsigned char*)mah, size); // ARP request에 대한 답장이므로, 0xFFFE
}
...
void sendARPRequest(char* id) { // Make ARP Contents and call sendFrame
	...
	mah->len = strlen(id);
	memcpy(mah->ethAddr, STATION_ADDR, sizeof(STATION_ADDR)); // 상대편에서 ARP reply시, 전송할 주소를 제공하기 위함
	memcpy(mah->data, id, sizeof(id)); //찾고자 하는 identifier의 이름
...
struct registered_dst* waitARPReply(char* id) {
	...
	while (1) {
		...
		receivedId = (unsigned char*)malloc(sizeof(mah->data) + 1);
		strcpy(receivedId, mah->data);
		...
		if (!strcmp(receivedId, id)) {
			...
			newEntry->id = (char*)malloc(strlen(mah->data));
			memcpy(newEntry->id, mah->data, strlen(mah->data));
			memcpy(newEntry->dst, mah->ethAddr, sizeof(mah->ethAddr)); //ethAddr: VM2의 MAC주소
			newEntry->next = NULL; //the next pointer of the lattest node of linked list is NULL
...
void sendFrame(unsigned char* dst, unsigned short type, unsigned char* data, int len) {
	...
	msgLength = len;
	memcpy(msgbuf_wrptr, data, msgLength);
	msgbuf_wrptr += msgLength;
...
ㅈ
	...
	rc = setsockopt(lsock, SOL_SOCKET, SO_BINDTODEVICE, interface, strlen(interface));
	if (0 != rc) {
