...
int main(int argc, char* argv[]) {
	...
	/* !!!! VM1 main code !!!! */
	// sendFrameTo(target, 0xFFFD, "2015130741 WonHyejin", strlen("2015130741 WonHyejin")); 
...
void receiveARPFrame(unsigned char* dst, unsigned char* arp) {
	...
	receivedId = (unsigned char*)malloc(strlen(ra->data)+1);
	...
	mah = (struct myarp_header*)malloc(sizeof(struct myarp_header) + name_size);
...
void sendARPRequest(char* id) { // Make ARP Contents and call sendFrame
	...
	printf("finished send ARP Request\n");
...
struct registered_dst* waitARPReply(char* id) {
	...
	while (1) {
		...
		if (!strcmp(receivedId, id)) {
			...
			memcpy(newEntry->dst, mah->ethAddr, strlen(mah->ethAddr));			
			newEntry->next = NULL;
